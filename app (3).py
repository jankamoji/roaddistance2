...
def sidebar():
    ...
    # Reference location input method
    ref_input_method = st.sidebar.radio("Select reference location by:", ["Coordinates", "Address Search", "Map Click"], key="ref_method")
    
    if ref_input_method == "Coordinates":
        ref_name = st.sidebar.text_input("Reference name", value=DEFAULT_REF["name"])
        ref_lat = st.sidebar.number_input("Reference latitude", value=DEFAULT_REF["lat"], format="%.6f")
        ref_lon = st.sidebar.number_input("Reference longitude", value=DEFAULT_REF["lon"], format="%.6f")
    
    elif ref_input_method == "Address Search":
        ref_name = st.sidebar.text_input("Reference name", value=DEFAULT_REF["name"])
        address = st.sidebar.text_input("Enter address (e.g., 'Bedburg, Germany')", value="Bedburg, Germany")
        if st.sidebar.button("🔍 Search Address"):
            try:
                params = {"q": address, "format": "json", "limit": 1}
                resp = requests.get(NOMINATIM_SEARCH, params=params, timeout=5)
                if resp.status_code == 200 and resp.json():
                    result = resp.json()[0]
                    ref_lat = float(result["lat"])
                    ref_lon = float(result["lon"])
                    st.sidebar.success(f"Found: {result.get('display_name', address)}")
                    st.session_state["ref_lat_temp"] = ref_lat
                    st.session_state["ref_lon_temp"] = ref_lon
                else:
                    st.sidebar.error("Address not found")
            except Exception as e:
                st.sidebar.error(f"Search error: {str(e)}")
        
        ref_lat = st.sidebar.number_input("Reference latitude", value=st.session_state.get("ref_lat_temp", DEFAULT_REF["lat"]), format="%.6f")
        ref_lon = st.sidebar.number_input("Reference longitude", value=st.session_state.get("ref_lon_temp", DEFAULT_REF["lon"]), format="%.6f")
    
    else:  # Map Click
        ref_name = st.sidebar.text_input("Reference name", value=DEFAULT_REF["name"])
        st.sidebar.info("📍 Click on the map below to set reference location")
        st.sidebar.write("Or use coordinates:")
        ref_lat = st.sidebar.number_input("Reference latitude", value=DEFAULT_REF["lat"], format="%.6f")
        ref_lon = st.sidebar.number_input("Reference longitude", value=DEFAULT_REF["lon"], format="%.6f")
...
def main():
    ...
    # Add interactive map for reference location
    st.subheader("🗺️ Reference Location Selector (Outbound)")
    col_map, col_info = st.columns([2, 1])
    
    with col_map:
        if _HAS_MAP:
            # Create interactive map for reference location selection
            m_ref = folium.Map(
                location=[51.0, 10.0],
                zoom_start=4,
                tiles="OpenStreetMap"
            ...
            # Add marker at current reference location
            if 'ref_lat' in st.session_state and 'ref_lon' in st.session_state:
                folium.Marker(
                    [st.session_state['ref_lat'], st.session_state['ref_lon']],
                    icon=folium.Icon(icon="star", color="green", prefix="fa"),
                    popup="Reference Location (Outbound)",
                    tooltip="Reference Location"
                ).add_to(m_ref)
                m_ref.location = [st.session_state['ref_lat'], st.session_state['ref_lon']]
                m_ref.zoom_start = 8
            else:
                folium.Marker(
                    [51.0126, 6.5741],
                    icon=folium.Icon(icon="star", color="green", prefix="fa"),
                    popup="Default: Bedburg, Germany",
                    tooltip="Reference Location"
                ).add_to(m_ref)
            
            Fullscreen().add_to(m_ref)
            st_folium(m_ref, height=400, use_container_width=True, key="ref_map")
        else:
            st.warning("Folium not available - map disabled")
    ...
    with col_info:
        ...
        **Reference Location (Outbound)**
        ...
        - Enter coordinates
        - Search by address
        - Or manually input coordinates
        
        Current:
        - Lat: {:.6f}
        - Lon: {:.6f}
        """.format(st.session_state.get('ref_lat', 51.0126), 
                   st.session_state.get('ref_lon', 6.5741)))
    ...
    if run:
        ...
        try:
            ...
            st.success(f"✅ Analysis complete! Processed {len(df_res)} sites")
            
            # Display full results table with all columns
            ...
            with col1:
                st.metric("Sites Analyzed", len(df_res))
            with col2:
                st.metric("API Calls", api_calls)
            with col3:
                st.metric("Cache Hits", len(st.session_state.get('route_cache', {})))
            with col4:
                st.metric("Execution Time", "~" + str(int((time.time() - st.session_state.get('analysis_start_time', time.time())) / len(df_res))) + "s/site")
